...

{
    ...
    ggsave("one-to-one/grouped-overlap.pdf", device='pdf', units="in", scale=S, width=W, height=H,
...

{
    ...
    ggsave("one-to-one/split-row.pdf", device='pdf', units="in", scale=S, width=W, height=H,
...

{
    ...
    data = rbind(data, data_wd, data_wps, data_wpss)
...

{
    ...
    ggsave("one-to-one/one-to-one-fft.pdf", device='pdf', units="in", scale=S, width=W, height=H,
        ggplot(data, aes(x=Input.matrix.rows,y=Kernel, color=alg, shape=alg)) +
        geom_point(size=point_size) +
        geom_line(linewidth=line_size) +
        xlab("Matrices sizes (log-scale)")+
