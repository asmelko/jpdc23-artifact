...

{
    ...
    data_basic = subset(data_basic, select=c(Kernel, Input.matrix.rows, Input.left.matrices, Input.right.matrices, rows_per_thread, right_matrices_per_thread, warps_per_thread_block))
    data = subset(data, select=c(Kernel, Input.matrix.rows, Input.left.matrices, Input.right.matrices, rows_per_thread, right_matrices_per_thread, warps_per_thread_block))
    ...
    ggsave("n-to-mn/multimat-right-split-row.pdf", device='pdf', units="in", scale=S, width=W, height=H,
        ...
        scale_y_log10(labels = sisec) +
        scale_x_log10(labels = function(x) paste0(x,"x",x), breaks=c(16,32,64,128,256,512)) +
        facet_wrap(~input, labeller=label_both, scales="free_y") +
        theme + background_grid() + theme(legend.position="bottom", axis.text.x = element_text(angle = -20, vjust=0.05))
    )
}
...

{
    ...
    data_fft2["Algorithm"] = "fft+prepare"
    ...
    ggsave("n-to-mn/n-to-mn-fft.pdf", device='pdf', units="in", scale=S, width=W, height=H,
        ggplot(data, aes(x=Input.matrix.rows,y=Kernel, color=Algorithm, shape=Algorithm)) +
        geom_point(size=point_size) +
        geom_line(linewidth=line_size) +
        xlab("Matrices size (log-scale)")+
...

{
    ...
    data["Matrices"][data["Matrices"] == "1x8"] <- "2x8"
    data["Matrices"][data["Matrices"] == "1x32"] <- "8x32"
